...
// 本地存储服务接口实现
class LocalStorageService: ObservableObject, StorageServiceProtocol {
    ...
    // 检查并创建文件
    private func ensureFileExists(fileName: String) -> Bool {
        let fileURL = getDocumentsDirectory().appendingPathComponent(fileName)
        let fileManager = FileManager.default
        
        if !fileManager.fileExists(atPath: fileURL.path) {
            print("文件不存在，尝试创建: \(fileName)")
            
            do {
                // 创建默认内容
                let defaultContent: Data
                if fileName == categoriesFileName {
                    defaultContent = try JSONEncoder().encode([Category]())
                } else if fileName == tasksFileName {
                    defaultContent = "{}".data(using: .utf8)!
                } else {
                    defaultContent = "{}".data(using: .utf8)!
                }
    ...
    func loadCategories() -> [Category] {
        // 确保文件存在
        guard ensureFileExists(fileName: categoriesFileName) else {
            return []
        }
    ...
    func loadTasks() -> [UUID: [Task]] {
        // 确保文件存在
        guard ensureFileExists(fileName: tasksFileName) else {
            return [:]
        }
        ...
        do {
            ...
            print("加载任务数据失败: \(error.localizedDescription)")
            return [:]
        }
    }
